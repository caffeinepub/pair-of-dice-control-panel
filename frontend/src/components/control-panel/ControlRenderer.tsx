...
export function ControlRenderer({ control, isEditMode }: ControlRendererProps) {
  ...
  const handleButtonPress = () => {
    ...
    console.log('[ControlRenderer] About to call sendGpioSignal with:', { decimalCode, state: 'on' });
    sendGpioSignal(decimalCode, 'on').catch((error) => {
      console.error('[ControlRenderer] ✗ sendGpioSignal error:', error);
    });
  ...
  const handleButtonRelease = () => {
    ...
    if (!buttonResetSentRef.current) {
      ...
      console.log('[ControlRenderer] About to call sendGpioSignal with:', { decimalCode, state: 'off' });
      sendGpioSignal(decimalCode, 'off').catch((error) => {
        console.error('[ControlRenderer] ✗ sendGpioSignal error:', error);
      });
  ...
  if (control.controlType === 'dial') {
    return (
      ...
      >
        ...
        <span className="text-xs text-white/70">Scroll or click arrows</span>
      </div>
    );
  }
