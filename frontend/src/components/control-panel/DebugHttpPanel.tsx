...
export function DebugHttpPanel() {
  ...
  const runTest = async (decimalCode: number, state: 'on' | 'off') => {
    ...
    try {
      ...
      setLastResult({ timestamp, success: false, error: errorMessage, decimalCode, state });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const previewBody = JSON.stringify({ button: validCode, value: selectedValue });
  ...
  return (
    <Card className="border-2 border-warning bg-warning/5">
      <CardHeader>
        ...
        <CardDescription>
          Test HTTP POST requests to localhost:3000/gpio independently
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg bg-muted p-3 font-mono text-xs space-y-1">
            <div>URL: <span className="text-primary">http://localhost:3000/gpio</span></div>
            <div>Method: <span className="text-primary">POST</span></div>
            <div>Content-Type: <span className="text-primary">application/json</span></div>
            <div>Body: <span className="text-primary">{previewBody}</span></div>
          </div>
        </div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <div className="font-mono text-xs space-y-1">
                  <div>Time: <span className="text-muted-foreground">{new Date(lastResult.timestamp).toLocaleTimeString()}</span></div>
                  <div>Body: <span className="text-primary">{JSON.stringify({ button: lastResult.decimalCode, value: lastResult.state })}</span></div>
                  {lastResult.success && (
                    <div className="text-green-600 mt-1">
                      ✓ Request delivered to localhost:3000/gpio
                    </div>
                  )}
                  {lastResult.error && (
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          ...
          <ul className="list-disc list-inside space-y-1 ml-2">
            ...
            <li>Click <strong>Send POST Request</strong> or use a quick test button</li>
            <li>Your GPIO server at <code>localhost:3000</code> must be running and allow CORS</li>
            <li>Open browser DevTools (F12) → Network tab to inspect the POST request</li>
            <li>If you see a CORS error, add <code>Access-Control-Allow-Origin: *</code> to your server</li>
          </ul>
        </div>
      </CardContent>
    </Card>
