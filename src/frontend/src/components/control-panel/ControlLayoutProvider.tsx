import type { Control, Layout } from "@/backend";
import { useActor } from "@/hooks/useActor";
import {
  ControlLayoutContext,
  type ControlLayoutContextValue,
} from "@/hooks/useControlLayout";
...
export function ControlLayoutProvider({ children }: { children: ReactNode }) {
  ...
  const updateControl = useCallback(
    (id: string, updates: Partial<ControlConfig>) => {
  ...
  const selectedControl =
    localControls.find((c) => c.id === selectedControlId) || undefined;
  ...
  return (
    <ControlLayoutContext.Provider value={contextValue}>
      {children}
  ...
  return {
    controls: layout.controls.map((control) => {
      const backendControl: Control = {
        ...
        decimalCodeOn:
          control.decimalCodeOn !== undefined
            ? BigInt(control.decimalCodeOn)
            : undefined,
        decimalCodeOff:
          control.decimalCodeOff !== undefined
            ? BigInt(control.decimalCodeOff)
            : undefined,
        decimalCodeUp:
          control.decimalCodeUp !== undefined
            ? BigInt(control.decimalCodeUp)
            : undefined,
        decimalCodeDown:
          control.decimalCodeDown !== undefined
            ? BigInt(control.decimalCodeDown)
            : undefined,
        decimalCodeLeft:
          control.decimalCodeLeft !== undefined
            ? BigInt(control.decimalCodeLeft)
            : undefined,
        decimalCodeRight:
          control.decimalCodeRight !== undefined
            ? BigInt(control.decimalCodeRight)
            : undefined,
  ...
  return {
    controls: backendLayout.controls.map((control) => {
      const frontendControl: ControlConfig = {
        ...
        color: "#dc2626",
        decimalCode:
          control.decimalCode !== undefined
            ? Number(control.decimalCode)
            : undefined,
        decimalCodeOn:
          control.decimalCodeOn !== undefined
            ? Number(control.decimalCodeOn)
            : undefined,
        decimalCodeOff:
          control.decimalCodeOff !== undefined
            ? Number(control.decimalCodeOff)
            : undefined,
        decimalCodeUp:
          control.decimalCodeUp !== undefined
            ? Number(control.decimalCodeUp)
            : undefined,
        decimalCodeDown:
          control.decimalCodeDown !== undefined
            ? Number(control.decimalCodeDown)
            : undefined,
        decimalCodeLeft:
          control.decimalCodeLeft !== undefined
            ? Number(control.decimalCodeLeft)
            : undefined,
        decimalCodeRight:
          control.decimalCodeRight !== undefined
            ? Number(control.decimalCodeRight)
            : undefined,
        sliderIsVertical: control.sliderIsVertical,
        radioGroupIsVertical: control.radioGroupIsVertical,
      };
      ...
      if (control.controlType === "radio" && control.radioOptions) {
        frontendControl.radioOptions = control.radioOptions.map(
          (label, index) => ({
            key: `option_${index + 1}`,
            label,
